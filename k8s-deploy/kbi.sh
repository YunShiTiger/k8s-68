...
while getopts i OPT; do
  ...
  i)
    ...
    echo -e "\033[32mKubernetes Finalization Installer\033[0m"
    echo -e "\033[32m欢迎使用KBI(Kubernetes Finalization Installer)\033[0m"
    echo -e "\033[32m========================================================================\033[0m"
    ...
    read -p "输入Kubernetes版本,默认v1.18.10(按回车跳过): " k8sVersion
...
EOF
  ...
  for node in ${nodeArray[@]}; do
    ...
    scp /etc/sysctl.d/kubernetes.conf root@$node:/etc/sysctl.d/
    ssh "$node" "yum install -y curl chrony sysstat conntrack ipvsadm ipset jq iptables psmisc iptables-services libseccomp && modprobe br_netfilter && sysctl -p /etc/sysctl.d/kubernetes.conf && mkdir -p /etc/kubernetes/pki/CA &> /dev/null"
    ...
    scp /etc/kubernetes/pki/CA/* "$node":/etc/kubernetes/pki/CA
...
deployHaproxyKeepalived() {
  ...
  for node in ${MasterIP[@]}; do ssh "$node" useradd keepalived_script; done
...
EOF
    ...
    if [ $? ]; then
      echo -e "\033[32m节点${node} Haproxy && Keepalived启动完成\033[0m"
    else
      echo -e "\033[31m节点${node} Haproxy && Keepalived启动失败，请执行systemctl status keepalived haproxy查看日志\033[0m"
    fi
    echo
  done
}
...
EOF
    ...
    if $(ssh "$node" "[[ -d /etc/kubernetes/pki/etcd/ ]]"); then
      echo -e "\033[32m节点$node 已存在/etc/kubernetes/pki/etcd/目录，跳过此步骤..........\033[0m"
    else
    ...
    if $(ssh "$node" "[[ -f /etc/kubernetes/pki/etcd/etcd-key.pem ]]"); then
      echo -e "\033[32m节点$node 已存在ETCD证书私钥文件，跳过此步骤..........\033[0m"
    else
      scp /etc/kubernetes/pki/etcd/* "$node":/etc/kubernetes/pki/etcd/
    fi
...
setKubectl() {
  ...
  rm -rf *.tar *.docker_tag

  for node in ${nodeArray[@]}; do
    scp /opt/kubernetes/server/bin/* "$node":/usr/local/bin/
...
EOF
  ...
  for node in ${MasterIP[@]}; do
    ssh "$node" "mkdir -p /etc/kubernetes/pki/admin /root/.kube/ &"
    scp /etc/kubernetes/pki/admin/admin* "$node":/etc/kubernetes/pki/admin/ 2>/dev/null
    scp /etc/kubernetes/pki/admin/admin.conf "$node":/root/.kube/config 2>/dev/null
...
EOF
  for node in ${nodeArray[@]}; do
    ...
    ssh "$node" "if [ ! -d /etc/kubernetes/pki/flannel/ ];then mkdir -p /etc/kubernetes/pki/flannel/ /run/flannel ; touch /run/flannel/docker;fi"

    if $(ssh "$node" systemctl status flanneld &>/dev/null); then
      echo -e "\033[32m$node Flanneld正在运行中，跳过复制可执行文件步骤..........\033[0m"
    else
      scp /opt/{flanneld,mk-docker-opts.sh} "$node":/usr/local/bin/
    fi

    if $(ssh "$node" "[[ -f /etc/kubernetes/pki/flannel/flannel.pem && -f /etc/kubernetes/pki/flannel/flannel-key.pem ]]"); then
      echo -e "\033[32m$node 已存在Flanneld证书文件，跳过此步骤..........\033[0m"
    else
      scp /etc/kubernetes/pki/flannel/flannel* "$node":/etc/kubernetes/pki/flannel/
    fi
    ...
    ssh "$node" "systemctl daemon-reload && systemctl start flanneld &> /dev/null"
...
EOF
  ...
  while ((nIndex < nodeArrayLen)); do
    sed -i "4 a\"${nodeArray[$nIndex]}\"," /etc/kubernetes/pki/apiserver/apiserver-csr.json
...
KUBE_API_ARGS="--admission-control=NamespaceLifecycle,LimitRanger,ServiceAccount,DefaultStorageClass,ResourceQuota,NodeRestriction \\
  ...
  --v=2"
EOF
    if $(ssh "$node" "[[ -d /etc/kubernetes/pki/apiserver ]]"); then
      echo -e "\033[32m$node 已存在/etc/kubernetes/pki/apiserver目录，跳过此步骤..........\033[0m"
    else
      ssh "$node" mkdir -p /etc/kubernetes/pki/apiserver/
    fi

    if $(ssh "$node" "[[ -d /etc/kubernetes/pki/bootstrap ]]"); then
      echo -e "\033[32m$node 已存在/etc/kubernetes/pki/bootstrap目录，跳过此步骤..........\033[0m"
    else
      ssh "$node" mkdir -p /etc/kubernetes/pki/bootstrap
    fi
    ...
    if $(ssh "$node" "[[ -f /etc/kubernetes/pki/bootstrap/token.csv ]]"); then
      echo -e "\033[32m$node 已存在/etc/kubernetes/pki/bootstrap/token.csv文件，跳过此步骤..........\033[0m"
    else
      scp /etc/kubernetes/pki/bootstrap/token.csv "$node":/etc/kubernetes/pki/bootstrap/
    fi

    if $(ssh "$node" "[[ -f /etc/kubernetes/pki/apiserver/apiserver-key.pem ]]"); then
      echo -e "\033[32m$node 已存在kube-apiserver证书私钥文件，跳过此步骤..........\033[0m"
    else
      scp /etc/kubernetes/pki/apiserver/apiserver* "$node":/etc/kubernetes/pki/apiserver/
    fi
...
EOF
  ...
  for node in ${MasterIP[@]}; do
    if $(ssh "$node" "[[ -d /etc/kubernetes/pki/controller-manager ]]"); then
      echo -e "\033[32m$node 已存在/etc/kubernetes/pki/controller-manager目录,跳过此步骤..........\033[0m"
    else
      ssh "$node" mkdir -p /etc/kubernetes/pki/controller-manager
    fi
    ...
    if $(ssh "$node" "[[ -f /etc/kubernetes/pki/controller-manager/controller-manager-key.pem ]]"); then
      echo -e "\033[32m$node 已存在kube-controller-manager证书私钥文件,跳过此步骤..........\033[0m"
    else
      scp /etc/kubernetes/pki/controller-manager/* "$node":/etc/kubernetes/pki/controller-manager/
    fi
...
EOF
  ...
  for node in ${MasterIP[@]}; do
    if $(ssh "$node" "[[ -d /etc/kubernetes/pki/scheduler ]]"); then
      echo -e "\033[32m$node 已存在/etc/kubernetes/pki/scheduler/目录,跳过此步骤..........\033[0m"
    else
      ssh "$node" mkdir -p /etc/kubernetes/pki/scheduler/
    fi
    ...
    if $(ssh "$node" "[[ -f /etc/kubernetes/pki/scheduler/scheduler-key.pem ]]"); then
      echo -e "\033[32m$node 已存在kube-scheduler证书私钥文件,跳过此步骤..........\033[0m"
    else
      scp /etc/kubernetes/pki/scheduler/* "$node":/etc/kubernetes/pki/scheduler/
    fi
...
EOF
    ...
    if $(ssh "$node" "[[ -f /etc/kubernetes/pki/bootstrap/bootstrap.kubeconfig ]]"); then
      echo -e "\033[32m$node 已存在kubelet bootstrap kubeconfig文件,跳过此步骤..........\033[0m"
    else
      scp /etc/kubernetes/pki/bootstrap/bootstrap.kubeconfig "$node":/etc/kubernetes/pki/bootstrap/
    fi
  ...
  if [ ! -e $curPath/images/pause-amd64-3.0.tar.gz ]; then
    ...
    docker save -o $curPath/images/pause-amd64-3.0.tar gcr.io/google_containers/pause-amd64:3.0
  fi
  for node in ${NodeIP[@]}; do
    scp /$curPath/images/pause-amd64-3.0.tar "$node":/tmp
...
EOF
  ...
  if [[ -f /etc/kubernetes/pki/proxy/proxy.kubeconfig ]]; then
    echo -e "\033[32m已存在kube-proxy文件,跳过此步骤..........\033[0m"
  else
...
EOF
    ...
    if $(ssh "$node" "[[ -d /etc/kubernetes/pki/proxy ]]"); then
      echo -e "\033[32m$node 已存在/etc/kubernetes/pki/proxy/目录,跳过此步骤..........\033[0m"
    else
      ssh "$node" "mkdir -p /etc/kubernetes/pki/proxy/" /var/lib/kube-proxy
    fi
    ...
    if $(ssh "$node" "[[ -f /etc/kubernetes/pki/proxy/proxy-key.pem ]]"); then
      echo -e "\033[32m$node 已存在kube-proxy证书私钥文件,跳过此步骤..........\033[0m"
    else
      scp /etc/kubernetes/pki/proxy/* "$node":/etc/kubernetes/pki/proxy/
    fi
